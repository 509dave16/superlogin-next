      ...
      try {
        ...
        if (userDoc.session) {
          ...
          var keysToAuthorize = Object.keys(userDoc.session).filter(function (k) {
            return userDoc.session[k] && userDoc.session[k] > Date.now();
          });
    ...
    var _ref12 = _asyncToGenerator(function* () {
      try {
        ...
        var _results$rows$reduce = results.rows.reduce(function (r, _ref13) {
          ...
          var userDoc = doc && doc[user];

          if (userDoc) {
            var _extends3;

            var _userDoc$session = userDoc.session,
                deleted = _userDoc$session[key],
                finalSession = _objectWithoutProperties(_userDoc$session, [key]);
