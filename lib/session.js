...
Session.prototype.storeToken = function (token) {
  ...
  if (!token.password && token.salt && token.derived_key) {
    return this._adapter.storeKey(tokenPrefix + ':' + token.key, token.expires - Date.now(), JSON.stringify(token)).then(function () {
      delete token.salt;
  ...
  return util.hashPassword(token.password).then(function (hash) {
