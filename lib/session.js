...
var Session = function Session(config) {
  ...
  if (sessionAdapter === 'redis') {
    adapter = (0, _RedisAdapter.default)(config);
  } else if (sessionAdapter === 'file') {
    adapter = (0, _FileAdapter.default)(config);
  } else {
    adapter = (0, _MemoryAdapter.default)();
  }

  return {
    confirmToken: function () {
      var _ref = _asyncToGenerator(function* (keys, password) {
        var entries = [];
        ...
        keys.forEach(function (key) {
          return entries.push(tokenPrefix + ":" + key);
        });
    ...
    storeToken: function () {
      var _ref4 = _asyncToGenerator(function* (token) {
        if (!token.password && token.salt && token.derived_key) {
          yield adapter.storeKey(tokenPrefix + ":" + token.key, token.expires - Date.now(), JSON.stringify(token));
          delete token.salt;
          delete token.derived_key;
        }

        var hash = yield _util.default.hashPassword(token.password);
