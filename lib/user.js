...
var user = function user(config, userDB, couchAuthDB, mailer, emitter) {
      ...
      try {
        ...
        if (saveDoc) {
          yield userDB.upsert(thisUser._id, function (oldUser) {
            var _ = oldUser.activity,
                oldData = _objectWithoutProperties(oldUser, ["activity"]);
  ...
  passwordConstraints = Object.assign({}, passwordConstraints, config.get().local.passwordConstraints);
  var userModel = {
    async: true,
