...
const merge = require('lodash.merge'); // regexp from https://github.com/angular/angular.js/blob/master/src/ng/directive/inupsert.js#L4
...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.validateEmail = email => new Promise(function ($return, $error) {
    var result;
    ...
    if (!email.match(EMAIL_REGEXP)) {
      return $return(Promise.resolve('invalid email'));
    }
    ...
    try {
      return userDB.query('auth/email', {
        ...
        if (result.rows.length === 0) {
          ...
          return $return(Promise.resolve('already in use'));
        }
  ...
  this.validateEmailUsername = email => new Promise(function ($return, $error) {
    var result;
  ...
  }.$asyncbind(this)); // Validation function for ensuring that two fields match
  ...
  this.socialAuth = (provider, auth, profile, req) => new Promise(function ($return, $error) {
    var results, emailFail, parseEmail, finalUser;
    let user, newAccount, action, baseUsername, ip;
    newAccount = false;
    req = req || {};
    ip = req.ip;
    ...
    try {
      ...
      return userDB.query('auth/' + provider, {
        ...
        if (results.rows.length > 0) {
          ...
          emailFail = () => new Promise(function ($return, $error) {
          ...
          if (emailUsername) {
            ...
            return self.validateEmailUsername(user.email).then(function ($await_19) {
              err = $await_19;

              if (err) {
                return $return(emailFail());
              }
            ...
            return self.validateEmail(user.email).then(function ($await_20) {
              err = $await_20;
        ...
        function $If_7() {
          ...
          if (newAccount) {
            return addUserDBs(user).then(function ($await_21) {
          ...
          function $If_8() {
            ...
            return self.logActivity(user._id, action, provider, req, user).then(function ($await_22) {
              return processTransformations(newAccount ? onCreateActions : onLinkActions, userDoc, provider).then(function ($await_23) {
                finalUser = $await_23;
                return userDB.upsert(finalUser._id, oldUser => merge({}, oldUser, finalUser)).then(function ($await_24) {
                  if (action === 'signup') {
                    emitter.emit('signup', user, provider);
                  }

                  return $return(Promise.resolve(user));
                }.$asyncbind(this, $Try_3_Catch), $Try_3_Catch);
              }.$asyncbind(this, $Try_3_Catch), $Try_3_Catch);
            }.$asyncbind(this, $Try_3_Catch), $Try_3_Catch);
          }
  ...
  this.logActivity = (user_id, action, provider, req, userDoc, saveDoc) => new Promise(function ($return, $error) {
    var logSize, entry;
    let promise, theUser;
    logSize = config.getItem('security.userActivityLogSize');

    if (!logSize) {
      return $return(Promise.resolve(userDoc));
    }
    ...
    if (!theUser) {
      ...
      return userDB.get(user_id).then(function ($await_25) {
        theUser = $await_25;
    ...
    function $If_10() {
      ...
      if (saveDoc) {
        return userDB.upsert(userDoc._id, oldUser => merge({}, oldUser, userDoc)).then(function ($await_26) {
  ...
  this.removeUserDB = (user_id, dbName, deletePrivate, deleteShared) => {
    ...
    return userDB.get(user_id).then(userDoc => {
      ...
      if (user.personalDBs && typeof user.personalDBs === 'object') {
        return new Promise(res => new Promise(function ($return, $error) {
          return $return(Object.keys(user.personalDBs).forEach(db => new Promise(function ($return, $error) {
            var type;

            if (user.personalDBs[db].name === dbName) {
              dbID = db;
              type = user.personalDBs[db].type;
              ...
              try {
                if (type === 'private' && deletePrivate) {
                  return dbAuth.removeDB(db).then(function ($await_27) {
                    return $return(res());
                  }.$asyncbind(this, $Try_4_Catch), $Try_4_Catch);
                }

                if (type === 'shared' && deleteShared) {
                  return dbAuth.removeDB(db).then(function ($await_28) {
                    return $return(res());
                  }.$asyncbind(this, $Try_4_Catch), $Try_4_Catch);
                }
  ...
  this.logoutUserSessions = (userDoc, op, currentSession) => new Promise(function ($return, $error) {
    var index;
    ...
    try {
      ...
      if (op === 'other' && currentSession) {
        index = sessions.indexOf(currentSession);
      ...
      if (sessions.length) {
        ...
        return session.deleteTokens(sessions).then(function ($await_29) {
          console.log('remove session keys'); // Remove the keys from our couchDB auth database

          return dbAuth.removeKeys(sessions).then(function ($await_30) {
            console.log('deauthorizeUser'); // Deauthorize keys from each personal database

            return dbAuth.deauthorizeUser(userDoc, sessions).then(function ($await_31) {
            ...
            }.$asyncbind(this, $Try_5_Catch), $Try_5_Catch);
          }.$asyncbind(this, $Try_5_Catch), $Try_5_Catch);
        }.$asyncbind(this, $Try_5_Catch), $Try_5_Catch);
      }
  ...
  this.remove = (user_id, destroyDBs) => new Promise(function ($return, $error) {
    var user, promises, userDoc, res;
    promises = [];
    ...
    try {
      return userDB.get(user_id).then(function ($await_32) {
        userDoc = $await_32;
        return self.logoutUserSessions(userDoc, 'all').then(function ($await_33) {
          res = $await_33;
          user = userDoc;
          ...
          return Promise.all(promises).then(function ($await_34) {
            return $return(userDB.remove(user));
  ...
  const addUserDBs = newUser => new Promise(function ($return, $error) {
    var promises, processUserDBs;
    let defaultPrivateDBs, defaultSharedDBs;

    // Add personal DBs
    if (!config.getItem('userDBs.defaultDBs')) {
      return $return(Promise.resolve(newUser));
    }

    promises = [];
    newUser.personalDBs = {};
    processUserDBs = (dbList, type) => new Promise(function ($return, $error) {
      return $return(Promise.all(dbList.map(userDBName => new Promise(function ($return, $error) {
        var dbConfig, finalDBName;
        dbConfig = dbAuth.getDBConfig(userDBName);
        return dbAuth.addUserDB(newUser, userDBName, dbConfig.designDocs, type, dbConfig.permissions, dbConfig.adminRoles, dbConfig.memberRoles).then(function ($await_35) {
          finalDBName = $await_35;
          delete dbConfig.permissions;
          delete dbConfig.adminRoles;
          delete dbConfig.memberRoles;
    ...
    defaultPrivateDBs = config.getItem('userDBs.defaultDBs.private');

    if (!Array.isArray(defaultPrivateDBs)) {
      defaultPrivateDBs = [];
    }

    return processUserDBs(defaultPrivateDBs, 'private').then(function ($await_36) {
      defaultSharedDBs = config.getItem('userDBs.defaultDBs.shared');

      if (!Array.isArray(defaultSharedDBs)) {
        defaultSharedDBs = [];
      }

      return processUserDBs(defaultSharedDBs, 'shared').then(function ($await_37) {
        return $return(Promise.resolve(newUser));
      }.$asyncbind(this, $error), $error);
    }.$asyncbind(this, $error), $error);
  }.$asyncbind(this));
