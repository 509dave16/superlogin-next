...
var DBAuth = require("./dbauth"); // regexp from https://github.com/angular/angular.js/blob/master/src/ng/directive/inupsert.js#L4
...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.create = function (form, req) {
    ...
    return user.process().then(function (result) {
      ...
      return userDB.upsert(finalNewUser._id, function (oldUser) {
        return _extends({}, oldUser, finalNewUser);
      });
    }).then(function (result) {
  ...
  this.socialAuth = function (provider, auth, profile, req) {
    ...
    var baseUsername;
    req = req || {};
    var ip = req.ip; // It is important that we return a Bluebird promise so oauth.js can call .nodeify()
    ...
    return BPromise.resolve().then(function () {
      ...
      return userDB.upsert(finalUser._id, function (oldUser) {
        return _extends({}, oldUser, finalUser);
      });
    }).then(function () {
      if (action === "signup") {
  ...
  this.createSession = function (user_id, provider, req) {
    ...
    return userDB.get(user_id).then(function (record) {
      ...
      return userDB.upsert(finalUser._id, function (oldDoc) {
        ...
        return _extends({}, oldDoc, finalUser);
      });
    }).then(function () {
    ...
    return promise.then(function (theUser) {
      ...
      if (saveDoc) {
        return userDB.upsert(userDoc._id, function (oldUser) {
          return _extends({}, oldUser, userDoc);
        }).then(function () {
          return BPromise.resolve(userDoc);
        });
      } else {
  ...
  this.removeUserDB = function (user_id, dbName, deletePrivate, deleteShared) {
    ...
    return userDB.get(user_id).then(function (userDoc) {
      ...
      if (update) {
        ...
        return userDB.upsert(user._id, function (oldUser) {
          if (oldUser.personalDBs) {
            delete oldUser.personalDBs[db];
          }

          return _extends({}, oldUser, user);
        });
      }
  ...
  this.logoutUserSessions = function (userDoc, op, currentSession) {
    ...
    if (sessions.length) {
      ...
      promises.push(session.deleteTokens(sessions)); // Remove the keys from our couchDB auth database

      promises.push(dbAuth.removeKeys(sessions)); // Deauthorize keys from each personal database
  ...
  function addUserDBs(newUser) {
    ...
    var processUserDBs = function processUserDBs(dbList, type) {
