...
var redis = Promise.promisifyAll(_redis.default);
...
var RedisAdapter = function RedisAdapter(config) {
  ...
  var unix_socket = redisConfig.unix_socket,
      url = redisConfig.url,
      port = redisConfig.port,
      host = redisConfig.host,
      options = redisConfig.options,
      password = redisConfig.password;
  var redisClient = unix_socket ? redis.createClient(unix_socket, options) : url ? redis.createClient(url, options) : redis.createClient(port || 6379, host || '127.0.0.1', options); // Authenticate with Redis if necessary
