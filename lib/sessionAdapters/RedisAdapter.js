...
var redis = _bluebird.default.promisifyAll(_redis.default);
...
var RedisAdapter = function RedisAdapter(config) {
  ...
  if (!config.getItem('session.redis.unix_socket')) {
    if (config.getItem('session.redis.url')) {
      ...
      redisClient = redis.createClient(config.getItem('session.redis.port') || 6379, config.getItem('session.redis.host') || '127.0.0.1', config.getItem('session.redis.options'));
    }
  } else {
