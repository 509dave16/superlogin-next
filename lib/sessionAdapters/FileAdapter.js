...
var path = require('path'); // BPromise.config({ warnings: false });
...
FileAdapter.prototype.getKey = function (key) {
  var now = Date.now();
  return fs.readJsonAsync(this._getFilepath(key)).then(function (session) {
...
FileAdapter.prototype._removeExpired = function () {// open all files and check session expire date
};