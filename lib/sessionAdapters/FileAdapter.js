...
var fs = Promise.promisifyAll(_fsExtra.default);

var FileAdapter = function FileAdapter(config) {
  var sessionsRoot = config.get().session.file.sessionsRoot;

  var _sessionFolder = _path.default.join(process.env.PWD, sessionsRoot);
    ...
    var _ref = _asyncToGenerator(function* (keys) {
      ...
      function () {
        var _ref2 = _asyncToGenerator(function* (key) {
          return yield fs.removeAsync(_getFilepath(key));
        });
