...
module.exports = function (configData, passport, userDB, couchAuthDB) {
  ...
  var middleware = new Middleware(passport); // Some extra default settings if no config object is specified
  ...
  var oauth = new Oauth(router, passport, user, config); // Seed design docs for the user database

  var userDesign = require('../designDocs/user-design');

  userDesign = util.addProvidersToDesignDoc(config, userDesign);
  seed(userDB, userDesign); // Configure Passport local login and api keys
