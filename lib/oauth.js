...
module.exports = function (router, passport, user, config) {
  ...
  function registerOAuth2(providerName, Strategy) {
    registerProvider(providerName, function (credentials, passport, authHandler) {
      passport.use(new Strategy(credentials, (req, accessToken, refreshToken, profile, done) => new Promise(function ($return, $error) {
        return $return(authHandler(req, providerName, {
  ...
  function registerTokenProvider(providerName, Strategy) {
    ...
    if (config.getItem(configRef + '.credentials')) {
      ...
      passport.use(providerName + '-token', new Strategy(credentials, (req, accessToken, refreshToken, profile, done) => new Promise(function ($return, $error) {
        return $return(authHandler(req, providerName, {
          accessToken: accessToken,
          refreshToken: refreshToken
        }, profile).then(done));
