...
module.exports = function (couchAuthDB) {
  ...
  this.deauthorizeKeys = (db, keys) => new Promise(function ($return, $error) {
    var secDoc, doc, changes;
    keys = util.toArray(keys);
    ...
    try {
      return db.get('_security').then(function ($await_3) {
        doc = $await_3;
        secDoc = doc;
        ...
        if (changes) {
          return putSecurityCouch(db, secDoc).then($return, $Try_1_Catch);
        } else {
          return $return(Promise.resolve(false));
        }
