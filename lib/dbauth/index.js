...
module.exports = function (config, userDB, couchAuthDB) {
  ...
  const deauthorizeUser = (userDoc, keys) => new Promise(function ($return, $error) {
    ...
    if (userDoc.personalDBs && typeof userDoc.personalDBs === 'object') {
      return $return(Promise.all(Object.keys(userDoc.personalDBs).map(personalDB => new Promise(function ($return, $error) {
        var db;
        ...
        try {
          db = new PouchDB(util.getDBURL(config.getItem('dbServer')) + '/' + personalDB, {
            skip_setup: true
          });
          return deauthorizeKeys(db, keys).then(function ($await_4) {
            return $return(Promise.resolve());
  ...
  this.removeDB = dbName => new Promise(function ($return, $error) {
    var db, res;
    ...
    try {
      db = new PouchDB(util.getDBURL(config.getItem('dbServer')) + '/' + dbName, {
        skip_setup: true
      });
      return db.destroy().then(function ($await_5) {
        res = $await_5;
        return $return(Promise.resolve());
