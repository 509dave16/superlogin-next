...
module.exports = function (config, userDB, couchAuthDB) {
      ...
      if (userDoc.personalDBs && typeof userDoc.personalDBs === 'object') {
        ...
        function () {
          var _ref2 = _asyncToGenerator(function* (personalDB) {
            try {
              var _db = new PouchDB(util.getDBURL(config.getItem('dbServer')) + '/' + personalDB, {
                skip_setup: true
              });

              console.log('deauthorizeKeys!', _db, keys);
              yield deauthorizeKeys(_db, keys);
  ...
  function () {
    var _ref3 = _asyncToGenerator(function* (dbName) {
      try {
        var _db2 = new PouchDB(util.getDBURL(config.getItem('dbServer')) + '/' + dbName, {
          skip_setup: true
        });

        var res = yield _db2.destroy();
